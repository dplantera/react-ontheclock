...
let indexedDbClient = new IDBClient("test_db");
...
test('test crud operations on an object', async () => {
    ...
    foundData = await storeData.findOne(1);
    expect(foundData.id).toBe(1);
...
test('create multiply objects and stores of different type', async () => {
    ...
    expect(created.id).toBe(1);
    expect(dataTwoStore.dbClient.version).toBe(3);

    const data3: DataThree = {attrNum: 3, attrBool: true};
    created = await dataThreeStore.create(data3);
    expect(created.id).toBe(1);
    expect(dataThreeStore.dbClient.version).toBe(4);
})
